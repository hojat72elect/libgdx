...
public class BoundingBox implements Serializable {
    ...
    public BoundingBox ext(Vector3 center, float radius) {
        return this.set(min.set(min(min.x, center.x - radius), min(min.y, center.y - radius), min(min.z, center.z - radius)), max.set(max(max.x, center.x + radius), max(max.y, center.y + radius), max(max.z, center.z + radius)));
    }
    ...
    public boolean contains(OrientedBoundingBox obb) {
        return contains(obb.getCorner000(tmpVector)) && contains(obb.getCorner001(tmpVector)) && contains(obb.getCorner010(tmpVector)) && contains(obb.getCorner011(tmpVector)) && contains(obb.getCorner100(tmpVector)) && contains(obb.getCorner101(tmpVector)) && contains(obb.getCorner110(tmpVector)) && contains(obb.getCorner111(tmpVector));
    }
