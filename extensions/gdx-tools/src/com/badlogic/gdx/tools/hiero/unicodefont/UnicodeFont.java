...
public class UnicodeFont {
    ...
    private Rectangle getGlyphBounds(GlyphVector vector, int index, int codePoint) {
        ...
        if (renderType == RenderType.Native) {
            if (bounds.width == 0 || bounds.height == 0) bounds = new Rectangle();
            else
    ...
    public String getFontFile() {
        if (ttfFileRef == null) {
            ...
            try {
                ...
                try {
                    // Java 7+.
                    font2D = Class.forName("sun.font.FontUtilities").getDeclaredMethod("getFont2D", java.awt.Font.class).invoke(null, font);
                } catch (Throwable ignored) {
                    font2D = Class.forName("sun.font.FontManager").getDeclaredMethod("getFont2D", java.awt.Font.class).invoke(null, font);
                }
