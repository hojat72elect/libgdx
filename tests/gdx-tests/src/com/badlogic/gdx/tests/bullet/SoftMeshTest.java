...
public class SoftMeshTest extends BaseBulletTest {
    ...
    public void create() {
        ...
        softBody = new btSoftBody(worldInfo, meshPart.mesh.getVerticesBuffer(false), meshPart.mesh.getVertexSize(), positionOffset, normalOffset, meshPart.mesh.getIndicesBuffer(false), meshPart.offset, meshPart.size, indexMap, 0);
    ...
    public void render() {
        if (world.renderMeshes) {
            MeshPart meshPart = model.nodes.get(0).parts.get(0).meshPart;
            softBody.getVertices(meshPart.mesh.getVerticesBuffer(true), meshPart.mesh.getVertexSize(), positionOffset, normalOffset, meshPart.mesh.getIndicesBuffer(false), meshPart.offset, meshPart.size, indexMap, 0);
