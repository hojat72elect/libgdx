...
public class OctreeTest extends GdxTestK implements ApplicationListener {
    ...
    private void generateGameObjects() {
        ...
        for (int i = 0, n = BOXES + REMOVE_BOXES; i < n; i++) {
            ...
            Vector3 center = new Vector3(-AREA_SIZE / 2 + random.nextFloat() * AREA_SIZE, random.nextFloat() * AREA_SIZE / 2, -AREA_SIZE / 2 + random.nextFloat() * AREA_SIZE);

            GameObject gameObject = new GameObject();
            Model modelBox = modelBuilder.createBox(width, height, depth, GL20.GL_TRIANGLES, objectMaterial, VertexAttributes.Usage.Position);
