...
public class AnimationTest extends GdxTest {
	...
	public void create () {
		...
		for (int i = 0; i < 100; i++) {
			cavemen[i] = new com.badlogic.gdx.tests.AnimationTest.Caveman(
				(float)Math.random() * com.badlogic.gdx.Gdx.graphics.getWidth(),
				(float)Math.random() * com.badlogic.gdx.Gdx.graphics.getHeight(), Math.random() > 0.5);
		}
