...
public class PairCacheTest extends BaseBulletTest {
    ...
    public void create() {
        ...
        for (int i = 0; i < BOXCOUNT; i++)
            world.add("collisionBox", BOX_X_MIN + dX * (float) Math.random(), BOX_Y_MIN + dY * (float) Math.random(), BOX_Z_MIN + dZ * (float) Math.random()).setColor(0.25f + 0.5f * (float) Math.random(), 0.25f + 0.5f * (float) Math.random(), 0.25f + 0.5f * (float) Math.random(), 1f);
    ...
    public boolean tap(float x, float y, int count, int button) {
        ...
        if (useFrustumCam) camera = frustumCam;
        else camera = overviewCam;
        return true;
    }
